...
def calculation(x, w, N, n0,  Z, l, V0, x0, D0, V1, x1, D1, pk, first, second):
    ...
    if first == True and second == False:
        ...
        xTrap      = np.array([i for i in xvar if i > x0f and i <= x1i])
        psiTrap    = np.array([psiarray[i] for i in range(0, len(xvar)) if xvar[i] > x0f and xvar[i] <= x1i] ) 
        if len(psiTrap) > 0 and len(xTrap) > 0:
            Trap = simpson(psiTrap**2, x=xTrap)
        else:
            Trap = 0.0
    else:
    ...
    if first == True and second == False:
        config_dir_name = f"(V0={V0}, D0={D0}, x0={x0})"
    elif first == True and second == True:
        config_dir_name = f"(V0={V0}, D0={D0}, x0={x0}, V1={V1}, D1={D1}, x1={x1})"
    else:
        config_dir_name = "free"

    config_dir_path = os.path.join(orbital_dir, config_dir_name)
    if not os.path.exists(config_dir_path):
        os.makedirs(config_dir_path)

    # Write info file
    info_file = os.path.join(config_dir_path, "info.txt")
    f = open(info_file, 'w')
    f.write(f'''TITLE: Comprehensive study of the one-electron system ({element[str(Z)]}) in {n0}{label[str(l)]} state
        under penetrable barrier-type potential \n''')
    f.write('*************************************************************************************************** \n\n')
    ...
    # Write density file
    density_file = os.path.join(config_dir_path, "one_particle_density_position.dat")
    ...
    # Write barrier file
    barrier_file = os.path.join(config_dir_path, "Barrier.dat")
