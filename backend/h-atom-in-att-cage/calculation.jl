...
function cal(R, Delta, v0, z, n, l, N, a0, aN, g, arr, slmdat)
    ...
    # Create the main orbital directory
    if !isdir(namedir)
        mkdir(namedir)
    end

    # Create the configuration subdirectory
    config_dir_name = "(V = $v0, R = $R , Delta = $Delta)"
    config_dir_path = joinpath(namedir, config_dir_name)
    if !isdir(config_dir_path)
        mkdir(config_dir_path)
    end

    data_file_name = joinpath(config_dir_path, "Correlated_data_(V = $v0, R = $R , Delta = $Delta).txt")
    ev, C = energy(N, l, z, v0, R, Delta, arr)
    ...
    script_dir = @__DIR__
    mesh_file_path = joinpath(script_dir, "Gauss_Leg", "mesh_200.txt")
    
    f = open(mesh_file_path, "r")
    # Declaring empty array to store the data files from mominfo
    roots  =  []
    weights = []
    ...
    f = open(data_file_name, "w")
    write(f, "***************************************************************************************************\n")
    write(f, "TITLE: Comprehensive Study of H-like Atom Under Penetrable Repulsive Barrier Potential  \n")
    write(f, "***************************************************************************************************\n")
    write(f, "AUTHOR                                   : Mr. Koustav D. Chakladar, Dr. Jayanta K. Saha \n")
    ...
    # position space density
    density_file = joinpath(config_dir_path, "one_particle_density_position_(V = $v0, R = $R , Delta = $Delta).dat")
    open(density_file, "w") do file
        lim = length(dencont_r)
        for i in 1:lim
            pii, dii = r[i], dencont_r[i]
            kprint = @sprintf("%0.10f                     %0.10E\n", pii, dii)
    ...
    # momentum space density
    momentum_file = joinpath(config_dir_path, "one_particle_density_momentum_(V = $v0, R = $R , Delta = $Delta).dat")
    open(momentum_file, "w") do file
        lim = length(dencont_p)
        for i in 1:lim
            pii, dii = p[i], dencont_p[i]
            kprint = @sprintf("%0.10f                     %0.10E\n", pii, dii)
    ...
    # compton profile
    compton_file = joinpath(config_dir_path, "compton_(V = $v0, R = $R , Delta = $Delta).dat")
    f = open(compton_file, "w")
    for i in 1:3000
        q = p[i]
        lim = length(p)
        pcomparr = p[i:lim]
    ...
    # barrier potential
    barrier_file = joinpath(config_dir_path, "Barrier_(V = $v0, R = $R , Delta = $Delta).dat")
    open(barrier_file, "w") do file
        lim = length(r)
        for i in 1:lim
            pii, dii = r[i], vpot(r[i], v0, R, Delta, l)
            kprint = @sprintf("%0.10f                     %0.10E\n", pii, dii)
